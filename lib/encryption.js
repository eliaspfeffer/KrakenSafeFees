...
const FALLBACK_ENCRYPTION_KEY = "abcdefghijklmnopqrstuvwxyz123456"; // Genau 32 ASCII-Zeichen = 32 Bytes

let key = process.env.ENCRYPTION_KEY || FALLBACK_ENCRYPTION_KEY;
...
if (Buffer.from(key, "utf-8").length !== 32) {
  ...
  // Wir verwenden immer den Fallback-Schlüssel, wenn die Umgebungsvariable nicht korrekt ist
  key = FALLBACK_ENCRYPTION_KEY;

  // Double-Check für den Fallback-Schlüssel
  if (Buffer.from(key, "utf-8").length !== 32) {
    throw new Error(
      "Kritischer Fehler: Der Fallback-Verschlüsselungsschlüssel hat nicht die richtige Länge (32 Bytes)."
    );
  }
}
